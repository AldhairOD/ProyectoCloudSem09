...
# Espera que configures .streamlit/secrets.toml:
# [app]
...
with tabs[0]:
    ...
    with colf1:
        txt = st.text_input("🔎 Buscar (nombre, apellidos, doc, correo, teléfono)", "")
    with colf2:
        estado_f = st.selectbox("Estado", ["— Todos —", "Activo", "Cesado", "Suspendido", "Vacaciones"])
    with colf3:
        dep_f = st.selectbox("Departamento", ["— Todos —"] + [dep_map[str(d["_id"])] for d in deps])
        dep_f_val = None
        if dep_f != "— Todos —":
            # obtener id desde el label
            dep_f_val = [k for k, v in dep_map.items() if v == dep_f][0]
    with colf4:
        per_page = st.selectbox("Por página", [5, 10, 20, 50], index=1)
...
with tabs[1]:
    ...
    with st.form("form_crear", clear_on_submit=True):
        ...
        with c1:
            doc_tipo = st.selectbox("Tipo de documento", ["DNI", "CE", "PAS"])
            doc_num = st.text_input("Número de documento")
        ...
        with c3:
            genero = st.selectbox("Género", ["M", "F", "X", "—"], index=3)
            f_nac = st.date_input("Fecha de nacimiento", value=None, format="YYYY-MM-DD")
            sueldo = st.text_input("Sueldo mensual (opcional)", value="")

        c4, c5, c6 = st.columns(3)
        with c4:
            dep = st.selectbox("Departamento", list(dep_options.keys()))
            dep_val = dep_options.get(dep)
        with c5:
            cargo = st.selectbox("Cargo", list(cargo_options.keys()))
            cargo_val = cargo_options.get(cargo)
        with c6:
            sede = st.selectbox("Sede", list(sede_options.keys()))
            sede_val = sede_options.get(sede)

        c7, c8, c9 = st.columns(3)
        with c7:
            f_ing = st.date_input("Fecha de ingreso", value=dt.date.today(), format="YYYY-MM-DD")
        with c8:
            tipo_contrato = st.selectbox("Tipo de contrato", ["Indeterminado", "Plazo Fijo", "Prácticas", "Outsourcing"])
        with c9:
            estado = st.selectbox("Estado", ["Activo", "Cesado", "Suspendido", "Vacaciones"], index=0)
...
with tabs[2]:
    ...
    # Selector de trabajador por búsqueda rápida
    q = st.text_input("Buscar por nombre/apellidos/doc", "")
    ...
    if candidatos:
        ...
        sel = st.selectbox("Selecciona un trabajador", ["— Selecciona —"] + list(opciones.keys()))
        if sel != "— Selecciona —":
            ...
            with st.form("form_update"):
                c1, c2, c3 = st.columns(3)
                with c1:
                    doc_tipo_u = st.selectbox("Tipo de documento", ["DNI", "CE", "PAS"], index=["DNI","CE","PAS"].index(t.get("doc_tipo","DNI")))
                    doc_num_u = st.text_input("Número de documento", value=t.get("doc_num",""))
                    nombres_u = st.text_input("Nombres", value=t.get("nombres",""))
                with c2:
                    apellidos_u = st.text_input("Apellidos", value=t.get("apellidos",""))
                    correo_u = st.text_input("Correo", value=t.get("correo","") or "")
                    telefono_u = st.text_input("Teléfono", value=t.get("telefono","") or "")
                with c3:
                    gen_val = t.get("genero") if t.get("genero") in ["M","F","X"] else "—"
                    genero_u = st.selectbox("Género", ["M", "F", "X", "—"], index=["M","F","X","—"].index(gen_val))
                    fn = t.get("fecha_nacimiento")
                    f_nac_u = st.date_input("Fecha de nacimiento", value=(fn.date() if fn else None), format="YYYY-MM-DD")
                    sueldo_u = st.text_input("Sueldo mensual (opcional)", value=str(t.get("sueldo_mensual","") or ""))

                c4, c5, c6 = st.columns(3)
                with c4:
                    dep_label = dep_map.get(str(t.get("id_departamento")), "— Selecciona —")
                    dep_u = st.selectbox("Departamento", list(dep_options.keys()), index=list(dep_options.keys()).index(dep_label) if dep_label in dep_options else 0)
                    dep_val_u = dep_options.get(dep_u)
                with c5:
                    cargo_label = cargo_map.get(str(t.get("id_cargo")), "— Selecciona —")
                    cargo_u = st.selectbox("Cargo", list(cargo_options.keys()), index=list(cargo_options.keys()).index(cargo_label) if cargo_label in cargo_options else 0)
                    cargo_val_u = cargo_options.get(cargo_u)
                with c6:
                    sede_label = sede_map.get(str(t.get("id_sede")), "— Selecciona —")
                    sede_u = st.selectbox("Sede", list(sede_options.keys()), index=list(sede_options.keys()).index(sede_label) if sede_label in sede_options else 0)
                    sede_val_u = sede_options.get(sede_u)

                c7, c8, c9 = st.columns(3)
                with c7:
                    fi = t.get("fecha_ingreso")
                    f_ing_u = st.date_input("Fecha de ingreso", value=(fi.date() if fi else dt.date.today()), format="YYYY-MM-DD")
                with c8:
                ...
                submitted_u = st.form_submit_button("Guardar cambios")
                if submitted_u:
                    data_u = {
                        "doc_tipo": doc_tipo_u,
...
with tabs[3]:
    ...
    qd = st.text_input("Buscar por nombre/apellidos/doc para eliminar", "")
    ...
    if candidatos_d:
        ...
        sel_d = st.selectbox("Selecciona un trabajador", ["— Selecciona —"] + list(opciones_d.keys()))
        if sel_d != "— Selecciona —":
